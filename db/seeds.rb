...
fish_csv = CSV.parse(File.read("catches.csv"), col_sep: "|", converters: :numeric)
...
fish_csv.each_with_index do |row, index|
  ...
  catch.image.attach({io: catch_image, filename: "image#{index}", content_type: 'image/png'})
end
...
bait_catches_csv.each do |row|
  bait_catch = BaitCatch.create(catch_id: row[1], bait_id: row[2])
end
